...
import { Upload, X, FileDown, AlertCircle, CheckCircle2, Loader2, UserPlus, FileUp } from 'lucide-react';
...
export const BulkImportModal: React.FC<BulkImportModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  // Manual state
  const [manualUser, setManualUser] = useState({ nome: '', email: '', senha: '', perfil: 'ESTUDANTE' });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [submittingManual, setSubmittingManual] = useState(false);

  // ... handleFileChange, handleImport, downloadTemplate remain the same ...
  ...
  const handleManualSubmit = async () => {
    const validated = userSchema.safeParse(manualUser);
    ...
    try {
      await createUser(manualUser, 'current-user-id'); // Re-visit how to get responsibleId
      onSuccess();
      onClose();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md" onClick={onClose}>
      <motion.div
      ...
      >
        <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
          <div className="space-y-1">
            <h2 className="text-2xl font-black text-slate-900 italic uppercase leading-none tracking-tighter">Gestão <span className="text-indigo-600">Usuários</span></h2>
        ...
        <div className="flex border-b border-slate-50">
          <button 
            className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 ${activeTab === 'import' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-400 hover:bg-slate-50'}`}
            onClick={() => setActiveTab('import')}
          >
            <FileUp className="w-4 h-4" /> Importação
          </button>
          <button 
            className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 ${activeTab === 'manual' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-400 hover:bg-slate-50'}`}
        ...
        <div className="p-10 space-y-8">
            ...
            <AnimatePresence mode="wait">
              {!result && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div 
                    className={`border-2 border-dashed rounded-[2.5rem] p-12 flex flex-col items-center justify-center transition-all group relative mt-4 ${
          ...
          {activeTab === 'manual' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              <input type="text" placeholder="Nome Completo" className="w-full p-4 border rounded-2xl" value={manualUser.nome} onChange={e => setManualUser({...manualUser, nome: e.target.value})} />
              {errors.nome && <p className="text-red-500 text-xs">{errors.nome}</p>}
              <input type="email" placeholder="E-mail" className="w-full p-4 border rounded-2xl" value={manualUser.email} onChange={e => setManualUser({...manualUser, email: e.target.value})} />
              {errors.email && <p className="text-red-500 text-xs">{errors.email}</p>}
              <input type="password" placeholder="Senha" className="w-full p-4 border rounded-2xl" value={manualUser.senha} onChange={e => setManualUser({...manualUser, senha: e.target.value})} />
              {errors.senha && <p className="text-red-500 text-xs">{errors.senha}</p>}
              <select className="w-full p-4 border rounded-2xl" value={manualUser.perfil} onChange={e => setManualUser({...manualUser, perfil: e.target.value})}>
                <option value="ESTUDANTE">Estudante</option>
                <option value="PROFESSOR">Professor</option>
                <option value="COORDENADOR">Coordenador</option>
              </select>
              {errors.perfil && <p className="text-red-500 text-xs">{errors.perfil}</p>}
              <button 
                onClick={handleManualSubmit} 
                disabled={submittingManual} 
                className="w-full py-5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-[1.5rem] font-black uppercase tracking-widest text-xs hover:from-indigo-700 hover:to-indigo-800 transition-all duration-300 disabled:opacity-50 shadow-lg shadow-indigo-200"
              >
                {submittingManual ? 'Salvando...' : 'Cadastrar Usuário'}
              </button>
            </motion.div>
          )}
        </div>
